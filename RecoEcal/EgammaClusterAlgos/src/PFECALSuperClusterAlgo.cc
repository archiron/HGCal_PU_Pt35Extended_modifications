...
namespace {
  ...
  struct IsClustered : public ClusUnaryFunction {
	...
	( (!dynamic_dphi && dphi < phiwidthSuperCluster_ ) || 
      ...
      bool pass=true;
      bool hgcalOK=true;      
      int det = x->the_ptr()->hitsAndFractions()[0].first.subdetId() ;
      switch( _type ) {
...
PFECALSuperClusterAlgo::PFECALSuperClusterAlgo() : beamSpot_(0) { 
  ...
  if( useFixedMatrixHGCAL_ ) {
    fixedMatrix_.reset( nullptr ); 
  } 
}
...
loadAndSortPFClusters(const edm::Event &iEvent) { 
  ...
  //initialize fixedMatrix for this event
  if (useFixedMatrixHGCAL_) {
    fixedMatrix_->setEvent(iEvent);
  }  
...
buildSuperCluster(CalibClusterPtr& seed,
  ...
  //apply fixed matrix energy assignment for HGCAL
  if( seed->the_ptr()->layer() == PFLayer::HGC_ECAL ) {
    if( useFixedMatrixHGCAL_ ) {    
      double enrj = fixedMatrix_->getEnergy(new_sc);
      new_sc.setEnergy(enrj);
    }
  }
