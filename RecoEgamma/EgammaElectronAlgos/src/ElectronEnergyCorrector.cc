...
///===================================================================
// Purpose: Class-based eta corrections for the ecal supercluster energy
// Author: Ivica Puljak - FESB, Split
// Author: Damir Leilas - Split
// Author: Stephanie Baffioni - LLR- Ecole Polytechnique
// Adaptation for CMS phase II upgrade (HGCAL)
...
ElectronEnergyCorrector::ElectronEnergyCorrector( EcalClusterFunctionBaseClass * crackCorrectionFunction )
 : crackCorrectionFunction_(crackCorrectionFunction), event_(0) 
 { 
  fixedMatrix_.reset( new HGCALSCFixedMatrixEnergy() ); 
 }

void ElectronEnergyCorrector::update(const edm::EventSetup& setup) 
 {
...
const reco::PFCluster* ElectronEnergyCorrector::getPFCluster(const reco::CaloCluster *cl) const {
  ...
  const reco::PFCluster *res = 0;  
  for (reco::PFClusterCollection::const_iterator itcl=clusters_->begin(); itcl!=clusters_->end();
   itcl++) {  
  ...
  //std::cout << "[ElectronEnergyCorrector::getPFCluster] found corresponding PFCluster " 
...
void ElectronEnergyCorrector::classBasedParameterizationUncertainty( reco::GsfElectron & electron )
 {
  ...
  error =  uncertainty.computeElectronEnergyUncertainty(electron.classification(),eleEta,brem,ecalEnergy) ;
...
void ElectronEnergyCorrector::hgcalSuperClusterCleanedEnergy (reco::GsfElectron & electron) 
 {

  //HGCAL supercluster cleaning
  //double energy = electron.superCluster()->energy() ;
  //double energy = electron.correctedEcalEnergy() ;
  double energy = 0. ;
  
  // first check we are in HGCAL
  ...
  int matrix_size_seed = 7;
  int matrix_size_sub = 5;
  int matrix_size = matrix_size_sub;
  
  // seed positionas a GlobalPoint
  GlobalPoint eleclPos(electron.superCluster()->seed()->position().x(),electron.superCluster()->seed()->position().y(),
   electron.superCluster()->seed()->position().z());
  //std::cout << "[ElectronEnergyCorrector::hgcalSuperClusterCleanedEnergy] new supercluster with energy " 
  // << electron.correctedEcalEnergy() << std::endl;
  ...
  for (reco::CaloCluster_iterator itcl=electron.superCluster()->clustersBegin(); itcl!=electron.superCluster()->clustersEnd(); itcl++) {
    ...
    // here refine the SC pruning by removing subclusters with wrong qxdphi
    double clet = cl->energy()/std::cosh(cl->eta());;
    GlobalPoint clPos(cl->position().x(),cl->position().y(),cl->position().z());
    ...
    if (electron.charge()*dphipca<-0.02 && clet<0.5) continue;
    //std::cout << "[ElectronEnergyCorrector::hgcalSuperClusterCleanedEnergy] adding new subcluster with energy " 
    // << (*itcl)->energy() << " and fixed matrix energy " << fixedMatrix_->getEnergy(cl,matrix_size) << std::endl;

    energy += fixedMatrix_->getEnergy(cl,matrix_size);
    
  }
  
  //std::cout << "[ElectronEnergyCorrector::hgcalSuperClusterCleanedEnergy] supercluster fixed matrix energy " << energy << std::endl;    
  // now apply overal calibration correction for the energy lost outside the window
  // only a global factor per category is applied
  // values taken from https://indico.cern.ch/event/391727/session/3/contribution/14/material/slides/0.pdf
  double calib = 1.;
  reco::GsfElectron::Classification elClass = electron.classification() ;
  if (elClass == reco::GsfElectron::GOLDEN) calib = 0.972;
  else if (elClass == reco::GsfElectron::SHOWERING) calib = 0.953;
  else if (elClass != reco::GsfElectron::GAP) calib = 0.962;
  energy /= calib;
  //std::cout << "[ElectronEnergyCorrector::hgcalSuperClusterCleanedEnergy] supercluster fixed matrix energy (calibrated) " << energy << std::endl;    
  
  electron.setCorrectedEcalEnergy(energy) ;
